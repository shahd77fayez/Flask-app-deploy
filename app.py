...
@app.route("/api/user/editProfile",methods=['POST'])
def UpdateProfile():
    user_id=session.get("user_id")
    email = request.json["email"]
    username = request.json["username"]
...
@app.route("/api/getarticlesbytype", methods=['GET'])
def get_articles():
...
@app.route("/api/OCRPredict", methods=['POST'])
def OCR_predict():
    user_id = session.get("user_id")
...
@app.route("/api/getfavbyid",methods=['GET'])
def get_favorite():
    user_id = session.get("user_id")
    if not user_id:
        return jsonify({'error': 'Unauthorized'}), 401
...
@app.route('/api/remove_favorite', methods=['DELETE'])
def remove_favorite():
    user_id = session.get("user_id")
...
@app.route('/api/getallcomments', methods=['GET'])
def get_comments():
...
def search():
    ...
    return jsonify([])
@app.route("/api/getHistById",methods=['GET'])
def get_history():
    user_id = session.get("user_id")
    if not user_id:
        return jsonify({'error': 'Unauthorized'}), 401
